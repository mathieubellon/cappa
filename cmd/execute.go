...
Write sql statements in a file (default '.cappa.sql') separate by newline and run this command.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if os.IsNotExist(err) {
			fmt.Println(chalk.Bold.TextStyle("File does not exists, create a .cappa.sql file"))
			os.Exit(0)
		}
		if err != nil {
		...
		primaryConn := createConnection(config, config.Database)
		...
		for _, sql := range Sqls {
			...
			if err != nil {
				log.Printf("Error executing the statement : %s", err)
			}
			executionCount += 1
		}
		if executionCount > 0 {
